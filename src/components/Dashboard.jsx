...
import { Search, Bell, User, Moon, Sun, Plus, Send, Sparkles, Home, MessageSquare, FolderKanban, BarChart3, ChevronRight, Menu, X, Upload, Trash2, Edit, Lightbulb, Loader2, Target, Users, Clock, Award } from 'lucide-react';
          ...
          <div className={`${dark?'bg-gray-900 border-gray-700':'bg-white border-gray-200'} rounded-2xl border p-6 max-w-md w-full`}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-4">
                {projectForm.logo && <img src={projectForm.logo} alt="Logo" className="w-16 h-16 rounded-lg object-cover border-2 border-purple-500"/>}
            ...
            <div className="flex space-x-3 mt-6">
              <button
                onClick={() => {setShowModal(false); setEditingProject(null); setProjectForm({name:'',description:'',logo:''});}}
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className={`p-6 rounded-2xl ${dark?'bg-gray-800/50 border-gray-700':'bg-white border-gray-200'} border`}>
                  ...
                  {projects.slice(-3).reverse().map((p,i)=>(
                    ...
                    >
                      {p.logo?<img src={p.logo} alt={p.name} className="w-10 h-10 rounded-lg object-cover"/>:<div className="w-10 h-10 bg-gradient-to-br from-purple-500 to-blue-500 rounded-lg flex items-center justify-center"><FolderKanban className="w-5 h-5 text-white"/></div>}
                  ...
                  {projects.map(p=>(
                    <div key={p.id} className={`p-6 rounded-2xl ${dark?'bg-gray-800/50 border-gray-700':'bg-white border-gray-200'} border hover:scale-105 transition group`}>
                      <div className="flex items-start justify-between mb-4">
                        {p.logo?<img src={p.logo} alt={p.name} className="w-12 h-12 rounded-lg object-cover"/>:<div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-blue-500 rounded-lg flex items-center justify-center"><FolderKanban className="w-6 h-6 text-white"/></div>}
                        <div className="flex space-x-1 opacity-0 group-hover:opacity-100">
                          ...
                          <button onClick={()=>{setEditingProject(p);setProjectForm({name:p.name,description:p.description,logo:p.logo});setShowModal(true);}} className={`p-2 rounded-lg ${dark?'hover:bg-gray-700':'hover:bg-gray-100'}`} title="Edit">
                            <Edit className="w-4 h-4 text-blue-400"/>
                          </button>
                          <button onClick={()=>deleteProject(p.id)} className={`p-2 rounded-lg ${dark?'hover:bg-gray-700':'hover:bg-gray-100'}`} title="Delete">
                            <Trash2 className="w-4 h-4text-red-400"/>
                          </button>
                        </div>
                      </div>
